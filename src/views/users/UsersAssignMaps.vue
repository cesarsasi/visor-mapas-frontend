...
<main class="container-1-1">
    ...
    <div class="container-header">
        ...
        <h2>Asignar mapas a {{user.name}}</h2>
    </div>
    ...
    <div class="container-body">
        ...
        <div class="search-tools" style="grid-template-columns: 1fr max-content;">
            ...
            <span>
                <button :disabled="!isAssignAllEnabled" class="btn btn-secondary" v-on:click="postManyRelMapUser()">Asignar todos</button>
            </span>
        </div>
<!-- 
        <div v-if="!showFilterResults" class="list-item-area">
            ...
            v-for="(item) in allMaps" >
                ...
                <span v-bind:key="item.index" class="list-item-sub-3">
                    <button v-bind:key="item.index" v-if="!item.isAssigned" class="btn btn-secondary" v-on:click="postRelMapUser(item)">Asignar</button>
                </span>
    ...
    <div class="container-list">
        ...
        <div class="search-tools">
            <span>
                <button :disabled="assignedMaps==null? true:false" class="btn btn-secondary" v-on:click="deleteManyRelMapUser()">No asignar todas</button>
            </span>
            <div></div>
            <span>
...
<!--         <div class="list-item-area">
            ...
            v-for="(item) in assignedMaps" >
                ...
                <span v-bind:key="item.index" class="list-item-sub-1">{{item.geo_url}}</span>
                <span v-bind:key="item.index" class="list-item-sub-3">
                    <button class="btn btn-secondary" v-on:click="deleteRelMapUser(item)">No asignar</button>
                </span>
                <span v-bind:key="item.index" class="list-item-separator"></span>
            </template>
...
export default{
    ...
    methods:{
        ...
        postRelMapUser: function(map){
            var that = this;
            this.$http.post(this.$store.getters.backendurl + 'api/v1/maps/' + map.id + '/users/' + this.$route.params.idUser, 
            {
                id_map: map.id,
                id_user: this.$route.params.idUser
            })
            .then(function (response) {
                //Actualizar datos
        ...
        deleteRelMapUser: function(map){
            var that = this;
            this.$http.delete(this.$store.getters.backendurl + 'api/v1/maps/' + map.id + '/users/' + this.$route.params.idUser)
            .then(function (response) {
                //Actualizar datos
                that.getAssignedMaps();
            })
        ...
        postManyRelMapUser: function(){
            ...
            for (let i = 0; i < list.length; i++) {
                ...
                if(!list[i].isAssigned) {
                    this.$http.post(this.$store.getters.backendurl + 'api/v1/maps/' + list[i].id + '/users/' +  this.user.id, 
                {
                    id_map: list[i].id,
        ...
        deleteManyRelMapUser: function(){
            var that = this;
            for (let i = 0; i < this.assignedMaps.length; i++) {

                this.$http.delete(this.$store.getters.backendurl + 'api/v1/maps/' + this.assignedMaps[i].id + '/users/' +  this.user.id, 
                {
